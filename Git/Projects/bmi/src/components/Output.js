...
class Output extends Component {
  toFeet(n) {
    ...
    let inches = Math.round((realFeet - feet) * 11);
    return feet + "'" + inches;
  }
  toLbs(kg) {
  ...
  render() {
    ...
    return (
      <div className="output">
        ...
        <h3>{weight}lbs</h3>
        <h3>{bmi}</h3>
        <h3 className={(this.props.data.bmiClass === "Obese") ? "danger": ""}>{bmiClass} {(this.props.data.bmiClass === "Obese") ? <a href='https://healthyeater.com/flexible-dieting'>What can I do?</a>: ""}</h3>
      </div>
    );
  }
}
